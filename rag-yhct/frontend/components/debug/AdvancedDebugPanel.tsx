...
export function AdvancedDebugPanel({ response }: AdvancedDebugPanelProps) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)] md:p-5">
      <details>
        <summary className="cursor-pointer text-base font-semibold text-slate-900">Gỡ lỗi nâng cao</summary>
        ...
        <details className="mt-2">
          <summary className="cursor-pointer text-sm font-medium text-slate-700">JSON bằng chứng thô</summary>
        ...
        <details className="mt-2">
          <summary className="cursor-pointer text-sm font-medium text-slate-700">JSON pipeline thô</summary>
          <pre className="mt-2 max-h-56 overflow-auto rounded-lg bg-slate-900 p-3 text-xs text-slate-200">
            {JSON.stringify(
              {
